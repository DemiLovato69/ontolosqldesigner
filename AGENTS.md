...
- Diagnostics: `FoundryRuntimeClient` logs runtime/upstream/access failures at `error` level (operation, host, exit code, and the Node bridge stderr tail) so they're visible in production (`LOG_LEVEL=error`). `sdk.mjs` writes the underlying SDK error (status/errorName/errorInstanceId, never the token) to stderr and maps 401/403 to `foundry_access_denied` with a scopes hint. A browse failure that returns `foundry_access_denied` is almost always missing OAuth scopes (`FOUNDRY_DEFAULT_SCOPES` + the OAuth client grant); reconnect after fixing.
- HTTP 406 gotcha: the `@osdk` SDK only sets a `Fetch-User-Agent` header, so Node's fetch sends `User-Agent: node`, which some Foundry gateways reject with 406 ("Unknown Error", no Palantir error body). `sdk.mjs` injects a real `User-Agent` (overridable via `FOUNDRY_HTTP_USER_AGENT`) on every Platform request via the `createPlatformClient(..., fetchFn)` hook.
