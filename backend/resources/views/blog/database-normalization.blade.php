...
@section('content')
    <article class="blog-post">
        ...
        <p>
            Problems: Alice's email is stored twice — if it changes, you must update every row. The Widget price is
            stored twice — a price change requires finding every row that references it. This is the kind of redundancy
            normalization eliminates.
        </p>

        <h2>First Normal Form (1NF)</h2>
        <p><strong>Rule:</strong> Every column must hold a single, atomic value. No repeating groups, no comma-separated
            lists in a cell.</p>
        ...
        <p>The <code>products</code> column contains multiple values. You can't query "all orders containing a Widget"
            without a <code>LIKE</code> hack.</p>
        ...
        <p><strong>Rule:</strong> The table must be in 1NF, and every non-key column must depend on the <em>entire</em>
            primary key — not just part of it. This only applies to tables with composite primary keys.</p>
        ...
        <p>The primary key is <code>(order_id, product_id)</code>. But <code>product_name</code> and
            <code>product_price</code> depend only on <code>product_id</code> — not on the full composite key. They're
            stored redundantly in every order line.</p>
...
product_id | product_name | product_price</code></pre>
        ...
        <p>Now <code>product_name</code> and <code>product_price</code> are stored once in <code>products</code>. A
            price change updates one row.</p>

        <h2>Third Normal Form (3NF)</h2>
        <p><strong>Rule:</strong> The table must be in 2NF, and no non-key column should depend on another non-key
            column (no transitive dependencies).</p>
        ...
        <p><code>department_name</code> depends on <code>department_id</code>, not on <code>employee_id</code>. It's a
            transitive dependency through a non-key column. Renaming the department requires updating every employee
            row.</p>
...
department_id | department_name</code></pre>
        ...
        <p>
            3NF is the right default for transactional databases. But sometimes you'll deliberately break the rules for
            performance:
        </p>
        <ul>
            <li><strong>Reporting and analytics</strong> — denormalized "wide" tables avoid expensive joins across many
                tables in read-heavy workloads.
            </li>
            <li><strong>Caching derived values</strong> — storing a pre-computed <code>order_total</code> avoids summing
                <code>order_items</code> on every page load, at the cost of keeping it in sync.
            </li>
            <li><strong>Historical snapshots</strong> — sometimes you <em>want</em> to store the product price at the
                time of purchase, not the current price. Denormalization is correct here by design.
            </li>
        </ul>
        <p>
            The key principle: normalize first, then denormalize deliberately and with documentation. Never denormalize
            out of laziness.
        </p>
        ...
        <nav aria-label="Related articles" style="margin-top:3rem; padding-top:2rem; border-top:1px solid var(--border-color);">
            ...
            <ul style="list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:0.5rem;">
                ...
                <li><a href="/blog/how-to-draw-er-diagram"
                       style="color:var(--color-primary); font-size:0.88rem; text-decoration:none;">How to Draw an ER
                        Diagram Step by Step &rarr;</a></li>
            </ul>
        </nav>
