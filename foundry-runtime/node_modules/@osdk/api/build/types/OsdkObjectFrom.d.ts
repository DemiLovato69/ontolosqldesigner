...
import type { ApplyModifiersToProps, PropertyModifierValue, PropsWithBoth, PropsWithOnlyMainValue, PropsWithOnlyReducers } from "./ontology/PropertyModifiers.js";
...
type OtHasNonLocalInterfaceImpl<
	FROM extends ObjectOrInterfaceDefinition,
	TO extends ObjectTypeDefinition,
	Implementations = ApiNameAsString<FROM> extends keyof NonNullable<CompileTimeMetadata<TO>["interfaceImplementations"]> ? NonNullable<CompileTimeMetadata<TO>["interfaceImplementations"]>[ApiNameAsString<FROM>] : undefined
> = Implementations extends undefined ? false : { [K in keyof Implementations] : Implementations[K] extends {
	type: "localProperty"
} ? never : K }[keyof Implementations] extends never ? false : true;
/**
...
> = IsNever<P> extends true ? N extends true ? never : PropertyKeys<Q> : IsAny<P> extends true ? PropertyKeys<Q> : P & PropertyKeys<Q>;
/**
...
type BuildModifiersFromP<P> = { [K in P as K extends `${infer Prop}:${string}` ? Prop : never] : K extends `${string}:${infer Mod extends PropertyModifierValue}` ? Mod : never };
