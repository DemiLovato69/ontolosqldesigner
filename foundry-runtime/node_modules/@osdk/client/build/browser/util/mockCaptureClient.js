 ...
 */
export function createMockCaptureClient(metadata = {
  ontologyRid: "unset"
}) {
  const fetchFn = vi.fn();
  ...
  const client = createMinimalClient(metadata, "https://foo", async () => "", {}, fetchFn);
  return {
    client,
    fetchFn
  };
 ...
 */
export function getLastObjectSetRequest(fetchFn) {
  const requestBody = fetchFn.mock.calls.reduceRight((acc, cur) => {
    if (acc) return acc;
...
//# sourceMappingURL=mockCaptureClient.js.map