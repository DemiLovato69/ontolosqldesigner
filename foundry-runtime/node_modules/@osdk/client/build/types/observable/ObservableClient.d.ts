...
import type { MediaMetadataObserveOptions, MediaMetadataPayload } from "./ObservableClient/MediaObservableTypes.js";
...
export type CacheEntry = CacheEntryBase & {
	type: "object"
} | CacheEntryBase & {
	type: "list"
	where?: unknown
...
export interface ObserveLinkCallbackArgs<T extends ObjectOrInterfaceDefinition> {
	...
	linkedObjectsBySourcePrimaryKey: ReadonlyMap<string | number, ReadonlyArray<Osdk.Instance<T, "$allBaseProperties">>>;
	isOptimistic: boolean;
	lastUpdated: number;
	fetchMore: () => Promise<void>;
	hasMore: boolean;
...
export interface ObservableClient extends ObserveLinks {
	...
	*/
	observeObject<T extends ObjectOrInterfaceDefinition>(apiName: T["apiName"] | T, pk: PrimaryKeyType<T>, options: ObserveOptions & Omit<ObserveObjectOptions<T>, "apiName" | "pk">, subFn: Observer<ObserveObjectCallbackArgs<T>>): Unsubscribable;
	/**
	...
	*/
	observeAggregation<
		...
		A extends AggregateOpts<T>,
		RDPs extends Record<string, SimplePropertyDef> = {}
	>(options: ObserveAggregationOptionsWithObjectSet<T, A, RDPs>, subFn: Observer<ObserveAggregationArgs<T, A>>): Promise<Unsubscribable>;
	/**
	...
	*/
	...
	canonicalizeOptions: <
		OS,
		T extends CanonicalizeOptionsInput<OS>
	>(options: T) => CanonicalizedOptions<T>;
	observeMediaMetadata(coords: MediaPropertyLocation, options: MediaMetadataObserveOptions, observer: Observer<MediaMetadataPayload>): Unsubscribable;
}
...
export type CanonicalizedOptions<T extends CanonicalizeOptionsInput<any>> = { [K in keyof T] : T[K] };
...
*/
export type ObservableClientLogLevel = "trace" | "debug" | "info" | "warn" | "error" | "fatal";
export declare function createObservableClient(client: Client, extraUserAgents?: () => string[], options?: ObservableClientOptions): ObservableClient;
