import type { ObjectOrInterfaceDefinition, Osdk, PageResult } from "@osdk/api";
...
*/
export declare class SpecificLinkQuery extends BaseListQuery<SpecificLinkCacheKey, SpecificLinkPayload, ObserveLinks.Options<ObjectOrInterfaceDefinition, string>> {
	#private;
	protected createPayload(params: CollectionConnectableParams): SpecificLinkPayload;
	/**
	* Register changes to the cache specific to SpecificLinkQuery
	*/
	protected registerCacheChanges(batch: BatchContext): void;
	constructor(store: Store, subject: Subject<SubjectPayload<SpecificLinkCacheKey>>, cacheKey: SpecificLinkCacheKey, opts: ObserveLinks.Options<ObjectOrInterfaceDefinition, string>);
	protected get rawSelect(): Canonical<readonly string[]> | undefined;
	get rdpConfig(): Canonical<Rdp> | undefined;
	get includeAllBaseObjectProperties(): boolean;
	/**
	...
	*/
	deleteFromStore(status: Status, batch: BatchContext): Entry<SpecificLinkCacheKey> | undefined;
	/**
	* Implements Query.maybeUpdateAndRevalidate to handle cache invalidation
	*/
	maybeUpdateAndRevalidate: (changes: Changes, _optimisticId: OptimisticId | undefined) => Promise<void>;
...
*/
export declare function isSpecificLinkCacheKey(key: CacheKey): key is SpecificLinkCacheKey;
