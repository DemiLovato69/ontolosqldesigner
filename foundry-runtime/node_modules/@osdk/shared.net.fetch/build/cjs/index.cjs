  ...
  return async function fetchOrThrow(url, requestInit) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        throw unknownError(fallbackMessage, response.status, error instanceof Error ? error : void 0);
      }
...
function createRetryingFetch(fetch2) {
  return fetchRetry__default.default(fetch2, {
    ...
    retryOn(attempt, error, response) {
      ...
      return !(status >= 200 && status < 300) && isRetryable(error) && attempt < MAX_RETRIES && response?.headers.get(QOS_RETRY_HINT_HEADER) !== DO_NOT_RETRY_HINT;
    }
  });
}
...
var QOS_RETRY_HINT_HEADER = "QoS-Retry-Hint";
var DO_NOT_RETRY_HINT = "do-not-retry";
