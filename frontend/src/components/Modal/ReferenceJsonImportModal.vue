    ...
    <div class="modal-overlay" @click.self="$emit('close')">
        <div class="modal-card reference-json-modal">
            <div class="modal-header">
                <span class="modal-title">Import Reference JSON</span>
            ...
            <div class="reference-json-modal__body">
                <p class="reference-json-modal__hint">
                    Paste a JSON Schema draft-07 object schema, or upload a JSON file. The schema title becomes the reference table name.
                </p>
                <textarea
                    v-model="content"
  ...
  "properties": {
    "customerId": { "type": "string" }
            ...
            <div class="modal-footer">
                <label class="btn btn-secondary reference-json-modal__upload" title="Upload JSON file">
                    <SvgIcon name="import" :size="15" />
                    <span>Upload JSON</span>
                    <input type="file" accept="application/json,.json" hidden @change="onFileChange" />
                </label>
...
const onFileChange = async (event) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
        $toast.error('Reference JSON files must be 10MB or smaller')
        return
    }
